...
SDK itself is distributed as maven package containing AAR, JARs and POM file declaring dependencies. As next step, add the local maven repo to project repositories. This repo will contain the SDK.

Update your project-level `settings.gradle` file and `dependencyResolutionManagement` section bny the following:
...
As next step, create a `repo` folder in project's root, copy `truemetricssdk.zip` into it and extract it, `repo` should look like this:
```json
...
You can find example snippets for creating a notification channel and asking permission to post notifications in the [Demo app](https://github.com/TRUE-Metrics-io/truemetrics_android_demo_app_p).
